...
from supabase_compat import router as supabase_router
...
app = FastAPI(title="My AI API", version="8.0.0", description="Self-hosted AI platform with Supabase-like auth, database REST, storage, realtime, functions, chat, memory, RAG, agents and media generation.")
origins = [x.strip() for x in os.getenv("CORS_ORIGINS", "*").split(",") if x.strip()]
app.add_middleware(CORSMiddleware, allow_origins=origins, allow_credentials=False, allow_methods=["GET", "POST", "PATCH", "PUT", "DELETE", "OPTIONS"], allow_headers=["Authorization", "Content-Type", "X-Request-ID", "X-User-ID"])
...
def rate_limit(request: Request) -> None:
    ...
    key = request.client.host if request.client else "unknown"; now = time.time(); bucket = _hits[key]
...
def enrich(messages: list[dict[str, Any]], uid: str):
    context=[]; memories=list_memories(uid)
    if memories: context.append("Private long-term memory:\n" + "\n".join(f"- {m['memory']}" for m in memories[:20]))
    query=content_to_text(messages[-1].get("content", "")); words={w.lower() for w in re.findall(r"[A-Za-z0-9]{3,}", query)}; matches=[]
    for f in list_files(uid):
        full=get_file(uid, f["id"]); text=full.get("content", "") if full else ""; score=sum(text.lower().count(w) for w in words)
...
@app.get("/health")
async def health(): return {"status":"ok" if await ollama_reachable() else "degraded","service":"my-ai-api","version":app.version,"model":DEFAULT_MODEL,"uptime_seconds":round(time.time()-STARTED_AT,1),"backend":"supabase-compatible-self-hosted"}
@app.get("/ready")
async def ready():
...
async def conversation_messages(request:Request,conversation_id:str):
    from db import conversation_owner
    if conversation_owner(conversation_id) != user_identity(request): raise HTTPException(404,"Conversation not found")
...
async def chat_impl(body:ChatRequest,uid:str):
    ...
    if body.conversation_id:
        from db import conversation_owner
        if conversation_owner(body.conversation_id) != uid: raise HTTPException(404,"Conversation not found")
