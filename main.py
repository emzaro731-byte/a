...
app = FastAPI(title="My AI API", version="2.0.0")
app.add_middleware(
    CORSMiddleware,
...
async def health() -> dict[str, Any]:
    try:
        ...
        return {"status": "ok", "ollama": "connected", "model": DEFAULT_MODEL, "version": "2.0.0"}
    except Exception as exc:
        return {"status": "degraded", "ollama": "unavailable", "error": str(exc), "version": "2.0.0"}
...
async def models() -> dict[str, Any]:
    ...
    return {
        "object": "list",
        "data": [{"id": item["name"], "object": "model", "owned_by": "local"} for item in data.get("models", [])],
    }
...
async def stream_from_ollama(request: ChatRequest, model: str, completion_id: str):
    ...
    try:
        ...
        yield f"data: {json.dumps({'error': {'message': f'AI model error: {detail}'}})}\n\n"
    except Exception as exc:
