...
ROUTES = {
    ...
    "reasoning": os.getenv("AI_GATEWAY_REASONING", "local:qwen2.5:3b"), "coding": os.getenv("AI_GATEWAY_CODING", "local:qwen2.5:3b"),
...
def _target(name: str | None, messages: list[dict[str, Any]] | None = None) -> tuple[str, str, str]:
...
async def _call(provider: str, model: str, body: GatewayRequest) -> dict[str,Any]:
    ...
    if cfg["kind"] == "ollama":
        answer=data.get("message",{}).get("content",""); prompt=data.get("prompt_eval_count",0) or 0; completion=data.get("eval_count",0) or 0
    else:
...
async def gateway_models():
    return {"object":"list","data":[{"id":k,"route":v,"profile":k} for k,v in ROUTES.items()]}
@router.get("/providers")
async def gateway_providers():
    return {"data":[{"id":n,"configured":bool(c["base_url"]) and (n=="local" or bool(c["api_key"])),"available":provider_available(n)} for n,c in PROVIDERS.items()]}
...
async def gateway_chat(body:GatewayRequest, authorization:str|None=Header(default=None)):
    ...
    provider,model,profile=_target(body.model,messages)
    ...
    try: result=await _call(provider,model,body)
        ...
        fp,fm,fprofile=_target("default",messages)
    ...
    result.update({"profile":profile,"latency_ms":round((time.perf_counter()-started)*1000,1)})
    return result
